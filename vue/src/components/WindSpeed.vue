...
const getWindSpeedByCity = async () => {
  try {
    const response = await axios.get('/wind-speed?city=' + city.value);
    formatedDate.value = Object.values(response.data["temps_formaté"]);
    windSpeed.value = Object.values(response.data["vitesse_vent"]);
...
onBeforeMount(getWindSpeedByCity);

const chartData = ref({
  labels: [],
  datasets: [
    {
      label: 'Vitesse de vent',
      data: [],
      borderColor: '#42A5F5',
      tension: 0.3,
      fill: false
...
const chartOptions = ref({
  ...
  plugins: {
    title: {
      display: true,
      text: 'Vitesse de vent à ' + city.value + ' en ' + new Date().getFullYear(),
      font: {
        size: 15,
        weight: 'bold'
      },
...
watchEffect(() => {
  if (formatedDate.value.length && windSpeed.value.length && city.value) {
    chartData.value = {
    ...
    chartOptions.value.plugins.title.text = 
      'Vitesse de vent à ' + city.value + ' en ' + new Date().getFullYear();
  }
});
</script>
